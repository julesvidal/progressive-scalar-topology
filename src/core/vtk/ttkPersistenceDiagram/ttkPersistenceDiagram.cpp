...
int ttkPersistenceDiagram::dispatch() {
  ...
  vector<tuple_t> *CTDiagram = (vector<tuple_t> *)CTDiagram_;
  if(UseProgressive) {
    persistenceDiagram_.setStartingDecimationLevel(StartingDecimationLevel);
    persistenceDiagram_.setStoppingDecimationLevel(StoppingDecimationLevel);
    persistenceDiagram_.setTimeLimit(TimeLimit);
    if(StoppingDecimationLevel > 0 || TimeLimit > 0) {
      // store state for resume only in a lower decimation level
      persistenceDiagram_.setIsResumable(IsResumable);
    }
  }
  if(computeDiagram_) {
    ...
    if(inputOffsets_->GetDataType() == VTK_INT) {
      if(UseProgressive) {
        ret = persistenceDiagram_.executeCPProgressive<VTK_TT, int>();
      } else {
    ...
    if(inputOffsets_->GetDataType() == VTK_ID_TYPE) {
      if(UseProgressive) {
        ret = persistenceDiagram_.executeCPProgressive<VTK_TT, vtkIdType>();
      } else {
...
#endif
    if(TimeLimit != 0) {
      // update this class stopping decimation level member to allow
      // to resume computation under time constraint by fiddling with
      // the StoppingDecimationLevel GUI widget
      this->StoppingDecimationLevel
        = persistenceDiagram_.getStoppingDecimationLevel();
    }
  } else if(resumeProgressive_) {
    persistenceDiagram_.resumeProgressive<VTK_TT>();
